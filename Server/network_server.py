...
class Server(object):
    def __init__(self, model_path):
        ...
        self.model.model.load_state_dict(torch.load('../model/fcn/models2.0/fcn_4l_60.pth'))
        self.model.optimizer.load_state_dict(torch.load('../model/fcn/models2.0/fcn_4l_opt_60.pth'))
        self.model.model.eval()

        self.data = torch.empty(0, 926)
        self.full = False
        self.input_mean, self.input_std = get_norm("../model/fcn/data/InputNorm.txt")
        self.input_mean, self.input_std = self.input_mean[0:926], self.input_std[0:926]
        self.output_mean, self.output_std = get_norm("../model/fcn/data/OutputNorm.txt")
    ...
    def forward(self, x):
        ...
        # temp_data = torch.Tensor(self.data)
        # data = self.model.model(temp_data.to(torch.device("cuda:0")))
        # print(data.shape)
